...
/* conn_tab functions */
static ssize_t dynamic_show_func(struct device *dev, struct device_attribute *attr, char *buf) {
	return snprintf(buf, PAGE_SIZE, "%d", num_of_dynamic_rules);
}

static ssize_t dynamic_store_func(struct device *dev, struct device_attribute *attr, const char *buf, size_t count) {
	return 1;
}

static DEVICE_ATTR(conn_tab, 0666, dynamic_show_func, dynamic_store_func);
...
dynamic_rule_link *curr_conn = NULL;
dynamic_rule_link *prev_conn = NULL;

static ssize_t read_dynamic(struct file *file, char __user * buffer, size_t length, loff_t * offset) {
	char tmp[100] = {0,};
	...
	if (curr_time - curr_conn->rule.timestamp > 25) {
	...
	// Source IP
	snprintf(tmp, PAGE_SIZE, "%d.%d.%d.%d ", NIPQUAD(curr_conn->rule.src_ip));\
	strcat(buffer, tmp);

	// Source Port
	snprintf(tmp, PAGE_SIZE, "%d ", ntohs(curr_conn->rule.src_port));
	strcat(buffer, tmp);	

	// Dest IP
	snprintf(tmp, PAGE_SIZE, "%d.%d.%d.%d ", NIPQUAD(curr_conn->rule.dst_ip));
	strcat(buffer, tmp);

	// Dest Port
	snprintf(tmp, PAGE_SIZE, "%d ", ntohs(curr_conn->rule.dst_port));
	strcat(buffer, tmp);

	// Protocol
	switch (curr_conn->rule.protocol) {
		case FTP:
			strcat(buffer, "FTP\n");
			break;
		case HTTP:
...
struct file_operations fops_dynamic = {
	.read = read_dynamic,
	.owner = THIS_MODULE
};
...
static int __init module_init_function(void) {
	...
	/* Create conn_tab device */
	major_dynamic = register_chrdev(0, "conn_tab", &fops_dynamic);

	m_dev_dynamic = MKDEV(major_dynamic, 0);

	dev_dynamic = device_create(m_c_fw, NULL, m_dev_dynamic, NULL, "conn_tab");

	device_create_file(dev_dynamic, &dev_attr_conn_tab);
...
static void __exit module_exit_function(void) {
	...
	/* unregister conn_tab */
	device_remove_file(dev_dynamic, &dev_attr_conn_tab);

	device_destroy(m_c_fw, m_dev_dynamic);

	unregister_chrdev(major_dynamic, "conn_tab");
